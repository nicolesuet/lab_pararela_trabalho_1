...
int main(int argc, char *argv[])
{
	...
	if (meu_ranque != 0)
	{
		MPI_Isend(&cont, 1, MPI_INT, 0, etiq, MPI_COMM_WORLD, &message);
		MPI_Wait(&message, &estado); /* Aguarda a mensagem ser enviada para garantir a conclusão do envio antes da operação de soma */
	}
	else
	{
