...
int main(int argc, char *argv[])
{
	...
	if (meu_ranque != 0)
	{
		MPI_Isend(&cont, 1, MPI_INT, 0, etiq, MPI_COMM_WORLD, &send_request);
		MPI_Wait(&send_request, &estado); /* Aguarda a mensagem ser enviada para garantir a conclusão do envio antes da operação de soma */
	}
	else
	{
		...
		{
        	MPI_Irecv(&cont, 1, MPI_INT, origem, etiq, MPI_COMM_WORLD, &receive_request);
			MPI_Wait(&receive_request, &estado);
